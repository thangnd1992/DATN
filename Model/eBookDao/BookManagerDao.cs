...
namespace Model.eBookDao
{
    public class BookManagerDao
    {
        ...
        {
            var bookManager = eb.BookManager.SqlQuery("dbo.GetBookBorrows @p0, @p1, @p2", query, borrowOnMin, borrowOnMax );
        ...
        {
            var bookManager = eb.BookManager.SqlQuery("dbo.GetBookPays @p0, @p1, @p2", query, payOnMin, payOnMax);
            return bookManager.OrderByDescending(x => x.Id).ToPagedList(page, pageSize);
        }
